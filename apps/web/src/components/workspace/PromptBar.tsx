...
import { Send, Loader2, History, ImagePlus, X, Sparkles, AlertCircle, HelpCircle, GitBranch } from "lucide-react";
...
export function PromptBar() {
  ...
  return (
    ...
    >
      ...
      {/* Variation picker — shown instead of interpretation banner when AI is uncertain */}
      {pendingVariations && !isAnalyzing && (
        <div className="mx-4 mt-2.5 rounded-lg border border-chroma-500/30 bg-chroma-500/5 overflow-hidden">
          <div className="flex items-center gap-2 px-3 py-2 border-b border-chroma-500/20">
            <GitBranch className="w-3.5 h-3.5 text-chroma-400 shrink-0" />
            <p className="flex-1 text-[11px] text-chroma-300 font-medium">
              I read this a few ways — which did you mean?
            </p>
            <button
          ...
          <div className="flex gap-px bg-chroma-500/10">
            {pendingVariations.map((v) => (
              <button
                key={v.label}
                onClick={() => selectVariation(v)}
                className="flex-1 flex flex-col gap-1 px-3 py-2.5 text-left hover:bg-chroma-500/10 transition-colors group"
              >
                <span className="text-[11px] font-semibold text-zinc-200 group-hover:text-chroma-300 transition-colors">
                  {v.label}
                </span>
                ...
                <span className="text-[9px] font-mono text-zinc-600 mt-0.5">
                  {[
                    v.adjustments.exposure !== 0 && `E ${v.adjustments.exposure > 0 ? "+" : ""}${v.adjustments.exposure.toFixed(1)}`,
                    v.adjustments.temperature !== 5500 && `${Math.round(v.adjustments.temperature)}K`,
                    v.adjustments.saturation !== 0 && `Sat ${v.adjustments.saturation > 0 ? "+" : ""}${Math.round(v.adjustments.saturation)}`,
                  ].filter(Boolean).join(" · ") || "Subtle adjustments"}
                </span>
              </button>
            ))}
          </div>
          <div className="px-3 py-1.5 border-t border-chroma-500/20">
            <button
              onClick={dismissVariations}
              className="text-[10px] text-zinc-600 hover:text-zinc-400 transition-colors"
            >
              None of these — I&apos;ll rephrase
            </button>
          </div>
        </div>
      )}

      {/* AI interpretation banner — hidden while variation picker is showing */}
      {lastInterpretation && !isAnalyzing && !pendingVariations && (
